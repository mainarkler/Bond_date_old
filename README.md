...
Внешние API, `OPENAI_API_KEY` и сетевые запросы для анализа **не используются**. Нужна локальная GGUF-модель, совместимая с `llama-cpp-python`; рекомендуемый вариант для русского языка — `Qwen2.5-3B-Instruct-Q4_K_M.gguf`. Скачайте/разместите файл модели заранее в окружении и задайте путь:

```bash
export LOCAL_LLM_MODEL_PATH=/opt/models/Qwen2.5-3B-Instruct-Q4_K_M.gguf
```

По умолчанию приложение ищет модель в `models/Qwen2.5-3B-Instruct-Q4_K_M.gguf`. Если файла либо `llama-cpp-python` нет, интерфейс покажет точную причину и перейдёт только к просмотру извлечённого текста.
